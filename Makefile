...
# Check if the FPGA environment was initialized
ifeq ($(FPGA),1)
...
$(EXEC): $(EXECOBJ) $(ALIB)
	$(CC) $(COMMON) $(CFLAGS) $^ -o $@ $(LDFLAGS) $(ALIB) $(AOCL_LINK_CONFIG) $(AOCL_LDFLAGS) $(AOCL_LDLIBS)
...
$(OBJDIR)%.o: %.c $(DEPS)
	$(CC) $(COMMON) $(CFLAGS) $(AOCL_COMPILE_CONFIG) -c $< -o $@
